...
class GRUTrajectoryPredictor(nn.Module):
    def __init__(self, horizon_steps: int, hidden_size: int = 128, layers: int = 2):
        super().__init__()
...
class CommunicationAwareObjective(nn.Module):
    ...
    def _log_snr(self, relative_xy: torch.Tensor) -> torch.Tensor:
        distance = torch.linalg.vector_norm(relative_xy, dim=-1).clamp_min(0.5)
        bearing = torch.atan2(relative_xy[..., 1], relative_xy[..., 0])
        log_gain = (
    ...
    def forward(
        self, predicted_xy: torch.Tensor, target_xy: torch.Tensor
    ) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        trajectory = functional.smooth_l1_loss(predicted_xy, target_xy)
        predicted_log_snr = self._log_snr(predicted_xy)
        target_log_snr = self._log_snr(target_xy)
        link = functional.smooth_l1_loss(predicted_log_snr, target_log_snr)
        gamma_threshold = -math.log(2 * self.outage_ber_threshold)
        target_outage = (target_log_snr < math.log(gamma_threshold)).float()
        logits = (
        ...
        total = trajectory + self.lambda_link * link + self.lambda_outage * outage
